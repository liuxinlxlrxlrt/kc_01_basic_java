package com.basicjava.f6_atm_system;
...
public class ATMSystem {
    public static void main(String[] args) {
        //1、定义账户类
        //2、定义一个集合容器，负载以后存储全部账户对象，进行先关的业务操作
    ...
    private static void login(List<Account> accounts, Scanner sc) {
        ...
        //1、判断账户集合汇总是否存在账户，如果不存在账户，登录功不能进行
        if (accounts.size()==0){
            System.out.println("对不起，当前系统汇总，无任何账户，请先开户，再来登录~~~");
            return;
        }
     ...
     * 展示登录后的操作页
     */
    private static void showUserCommand(Scanner sc,Account acc,List<Account> accounts) {
        while (true) {
     ...
     * @param accounts
     */
    private static boolean deleteAccount(Account acc, Scanner sc, List<Account> accounts) {
        ...
        switch (rs){
            case "y":
                //真正销户
                if (acc.getMoney()>0){
                    System.out.println("您账户中还有前没有取完，不允许销户~~");
                }else {
                    accounts.remove(acc);
     ...
     */
    private static void transferMoney(Scanner sc, Account acc, List<Account> accounts) {
        ...
        //3、真正开始转账
        while (true) {
            System.out.println("请请您输入对方账户的卡号：");
            String cardId = sc.next();
            ...
            if (account==null){
                System.out.println("对不起，您输入对方的这个账户不存在~~~");
            }else {
                //这个账户对象存在了，继续认证对方的姓氏
    ...
    private static void drawMoney(Account acc, Scanner sc) {
        ...
        while (true) {
            ...
            if (money>acc.getQuotaMoney()){
                System.out.println("对不起，当前取款金额超过每次取款金额，每次最多可取："+acc.getQuotaMoney());
            }else {
                //没有超过当次限额
                //4、判断是否超过了账户的总余额
                if (money>acc.getMoney()){
                    ...
                    //可以取钱了
                    System.out.println("恭喜您，取现："+money+"元成功！");
                    //更新余额
                    acc.setMoney(acc.getMoney()-money);
                    //取现结束了
                    showAccount(acc);
     ...
     * @return  账户对象 | null
     */
    private static Account getAccountByCardId(String cardId,List<Account> accounts){
